---
description: Analyze session for skill learnings and propose improvements (self-learning system)
argument-hint: [--skill <name>] [--dry-run]
allowed-tools: Read, Glob, AskUserQuestion, Task, Skill
---

Activate the heal-skills skill to analyze this session for learnings.

Look for:
1. Skills that were activated during this session
2. Errors, user corrections, or retry patterns that indicate learning opportunities
3. Propose specific updates to improve those skills

$ARGUMENTS
